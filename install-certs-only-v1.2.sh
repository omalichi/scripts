...
addPrefixAndSuffixForCerts()
{
	...
	do
		ready_certs+=($CERT_PREFIX$i$CERT_SUFFIX)
	done
...
addCerts()
{
	...
	do
		if [ -s "$i" ]; then
			if ! [ -s "$i.$DEFAULT_BACKUP_FILE_SUFFIX" ]; then
				backupFile i
				...
				echo "File '$i' was updated already."
			fi
		else
			echo "File '$i' was not found."
		fi
	done
...
showAllSettings()
{
	...
	local -n certsDirsArr=$2	
		
	if [ -z "$4" ]; then
		local -n backupFileSuffix=DEFAULT_BACKUP_FILE_SUFFIX
	else
		local -n backupFileSuffix=$4
	fi	
	...
	do
		if [ -s "$i" ]; then
			if [ -s "$i.$backupFileSuffix" ]; then
				echo "========================================================"
				echo "$i:"
				echo "========================================================"
				ls -l $i
				ls -l $i.$backupFileSuffix
				echo ""
				echo ""				
			fi
		fi
	...
	do
		if [ -d "$i" ]; then
			...
			if [ "$?" -eq 0 ]; then
				...
				echo "$i:"
				echo "========================================================"
				ls $i | grep $DEFAULT_ADDED_BY_PREFIX
				echo ""
				echo ""
			fi
		fi
